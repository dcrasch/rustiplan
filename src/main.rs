...
fn main() {
    dioxus_desktop::launch_cfg(
        ...
        Config::new()
            ...
            .with_custom_head(format!("<style>{}</style>", include_str!("assets/styling.css"))),
);
...
pub fn Cell(cx: Scope<CellProps>) -> Element {
	...
	div { class: "cell {editing}",
	      ondblclick: move |_| { println!("click"); is_editing.set(true) },                
	      label {
                  "{contents}"
              },
	      is_editing.then(|| {
...
fn app(cx: Scope) -> Element {
    ...
    let rows = (0..10).map(move |row_index| {
        let cells = ('A'..='E').enumerate().map(move |(column_index,_column)| {
            let children = data.read().get(row_index).unwrap().get(column_index).unwrap().to_string();
            rsx! {
                td {
